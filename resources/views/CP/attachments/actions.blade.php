@if (Auth::user()->can($config['singular_key'] . '_edit'))
    <div class="d-flex align-items-center gap-2">
           <!-- Edit Button -->
           <a href="{{ route($config['full_route_name'] . '.edit', ['_model' => $_model->id, 'attachable_type' => $attachable_type, 'attachable_id' => $attachable_id]) }}"
        ...
        <!-- Delete Button -->
        <a href="{{ route($config['full_route_name'] . '.delete', ['_model' => $_model->id, 'attachable_type' => $attachable_type, 'attachable_id' => $attachable_id]) }}"
