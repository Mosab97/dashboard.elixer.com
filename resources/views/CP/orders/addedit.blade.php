...
@section('content')
                ...
                <ul id="myTab"
                    ...
                    <li class="nav-item mt-2">
                        <a class="nav-link text-active-primary ms-0 me-6 px-2 py-5 " data-bs-toggle="tab"
                            data-bs-target="#kt_tab_order_items" href="#kt_tab_order_items">
                            <span class="svg-icon svg-icon-2 me-2"></span>
                            {{ t($config['children']['order_items']['singular_name']) }}
                        </a>
                    </li>
                </ul>
                <!--end::Navs-->
    ...
    <form action="{{ route($config['full_route_name'] . '.addedit', ['Id' => $_model->id ?? null]) }}" method="POST"
        ...
        <div class="tab-content" id="myTabContent">
            ...
            @if ($_model->exists)
                <div class="tab-pane fade " id="kt_tab_order_items" role="tabpanel">
                    @include('CP.orders.tabs.order_items.index')
                </div>
            @endif
