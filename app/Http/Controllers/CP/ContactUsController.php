...
use App\Http\Requests\CP\ContactUsRequest;
use App\Models\ContactUs;
use App\Services\Filters\ContactUsFilterService;
...
class ContactUsController extends Controller
{
    ...
    public function __construct(
        ContactUs $_model,
        ContactUsFilterService $filterService,
    ) {
        $this->config = config('modules.contact_us');
        $this->_model = $_model;
        ...
        if ($request->isMethod('POST')) {
            ...
            return DataTables::eloquent($items)
                ...
                ->editColumn('message', function ($item) {
                    return '<p title="'.$item->message.'">' . \Illuminate\Support\Str::limit($item->message ?? 'N/A', 100) . '</p>';
                })
                ...
                ->rawColumns(['name', 'email', 'phone', 'subject', 'message', 'action'])
                ->make(true);
        }
    }
    ...
    public function edit(Request $request, ContactUs $_model)
    {
    ...
    public function addedit(ContactUsRequest $request)
    {
    ...
    public function delete(Request $request, ContactUs $_model)
    {
        try {
            DB::beginTransaction();
