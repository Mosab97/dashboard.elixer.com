...
use App\Http\Requests\CP\CouponRequest;
use App\Models\Coupon;
use App\Services\Filters\CouponFilterService;
...
class CouponController extends Controller
{
    ...
    public function __construct(
        Coupon $_model,
        CouponFilterService $filterService,
    ) {
        $this->config = config('modules.coupons');
        $this->_model = $_model;
        ...
        if ($request->isMethod('POST')) {
            ...
            return DataTables::eloquent($items)
                ...
                ->editColumn('discount', function ($item) {
                    return $item->discount ?? 'N/A';
                })
                ->editColumn('expiry_date', function ($item) {
                    return $item->expiry_date ?? 'N/A';
                })
                ...
                ->rawColumns(['code', 'discount', 'expiry_date', 'active', 'action', 'created_at'])
                ->make(true);
        }
    }
    ...
    public function edit(Request $request, Coupon $_model)
    {
    ...
    public function addedit(CouponRequest $request)
    {
    ...
    public function delete(Request $request, Coupon $_model)
    {
        try {
            DB::beginTransaction();
