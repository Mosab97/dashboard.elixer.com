...
class RealResultFilterService
{
    ...
    {
        Log::info('Starting to apply real result filters with params:', $params);

        foreach ($params as $key => $value) {
            if ($value !== null && $value !== '') {
                Log::info("Applying real result filter for: $key with value:", ['value' => $value]);
                $this->applyFilter($query, $key, $value);
            } else {
                Log::info("Skipping real result filter for: $key as the value is null or empty.");
            }
        }

        logQuery($query);
        Log::info('All real result filters applied.');
    ...
    protected function applyFilter($query, $key, $value)
    {
        switch ($key) {
            ...
            default:
                Log::warning("Unknown real result filter key: $key");
                break;
        }
    }
